...
def ManhattanDistance(state: State, i):   # i is index of empty blank in parent state
...
def initialManhattan(initialstate: State):    # Manhattan function finds the heuristic value for the initial state only
    for i in range(9):
...
def pos(index): return index // 3, index % 3  # given index it returns the x,y coordinates

def initialEuclidean(initialState: State): # find initial Euclidean distance for the initial state
    for i in range(9):
        tile = int(initialState.value[i])
        if tile != 0:
            if tile != i:   # if tile == its index then skip
                tilePosx, tilPosy = pos(tile)  # goal position
                iPosx, iPosy = pos(i)          # current position
...
def EucildeanDistance(state: State, i):
    state.heuristics = copy(state.parent.heuristics)
    tile = int(state.value[i])
    tilePosx, tilPosy = pos(tile)  # goal position
    iPosx, iPosy = pos(i)          # current position
